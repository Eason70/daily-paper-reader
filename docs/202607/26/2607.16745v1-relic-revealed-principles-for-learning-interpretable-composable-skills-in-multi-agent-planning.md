...
tldr: 多智能体规划中，智能体需在保护隐私下提升决策技能并协调。现有方法依赖集中优化或共享策略，不适合隐私受限场景。本文提出RELIC框架，每个智能体通过私有LLM引导搜索优化程序化技能，成功行为被抽象为可移植原则供其他智能体实例化。这实现了无需共享代码的技能学习与跨智能体迁移，为隐私保护的多智能体规划引入新范式。
source: arxiv
...
motivation: 现有方法依赖集中优化或共享策略，不适用于隐私受限的多智能体合作场景。
method: 每个智能体通过私有LLM引导搜索优化程序化技能，可信编排器仅评估团队表现，成功行为抽象为原则供其他智能体实例化。
result: 智能体可基于抽象原则重用和组合技能，无需暴露内部策略。
conclusion: RELIC为隐私保护的多智能体规划提供了可解释、可组合的技能学习新范式。
---

## 摘要
当智能体必须提升专门的决策技能同时保持其内部实现私密时，多智能体规划变得更为困难。当智能体独立开发、暴露不同接口和功能，且必须在不共享可执行策略的情况下协调时，就会出现这种情况。先前的研究大多假设集中优化、共享策略访问或通用技能表示，因此不适合隐私约束的合作。我们提出了RELIC，一个通过揭示原则学习可解释和可组合技能的框架。每个智能体通过私有的LLM引导搜索优化其自身的程序化技能，而受信任的协调器仅通过团队级性能评估提议的更新。成功的行为不广播为代码；相反，它们被抽象为可移植的原则，其他智能体可以在自己的接口中实例化这些原则，并与本地策略重新组合。这分离了协调与实现共享，实现了异质技能签名下的跨智能体迁移。因此，RELIC为多智能体规划中的隐私保护技能学习和协调引入了一种新范式。
